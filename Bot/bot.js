...
exports.start = function(SETUP) {
  ...
  const UpdateEmbed = function() {
    let dot = TICK_N % 2 === 0 ? 'Crime Town' : 'Roleplay';
    let embed = new Discord.RichEmbed()
    .setAuthor("CTRP Server Status", "https://cdn.discordapp.com/attachments/804818012867526656/812904950085844992/LOGO.png")
    .setColor(0x2894C2)
    .setFooter(TICK_N % 2 === 0 ? '🔵 CTRP' : '🟢 CTRP')
    .setTimestamp(new Date())
    .addField('\n\u200b\nCrime Town Roleplay','```Fivem:-cfx.re/join/5vo4p7``` ```Team Speak:-13.234.118.196```\n\u200b\n',false)
    if (STATUS !== undefined)
    {
  ...
  bot.on('ready',() => {
    ...
    bot.user.setActivity('CTRP Server Status', { type: 'WATCHING' })
    // bot.user.setActivity('CTRP Server Status',{'url':'https://www.twitch.tv/RoqueTV','type':'STREAMING'});
